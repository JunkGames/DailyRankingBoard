...
class PlayerArmorStandData(private val uuid: UUID) {
    ...
    private val leaderboardGameDummyUpArrow = ArmorStandUtil.createHologram(instance.getGameSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 7.5), null)
    val leaderboardGameUpArrow = ArmorStandUtil.createHologram(instance.getGameSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 6.5), "${ChatColor.GOLD}▲△▲")
    private val leaderboardGameCurrent = ArmorStandUtil.createHologram(instance.getGameSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 5.5), "") // GOLD + BOLD + game name
    val leaderboardGameDownArrow = ArmorStandUtil.createHologram(instance.getGameSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 4.5), "${ChatColor.GOLD}▼▽▼")

    private val leaderboardMapDummyUpArrow = ArmorStandUtil.createHologram(instance.getMapSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 7.5), null)
    val leaderboardMapUpArrow = ArmorStandUtil.createHologram(instance.getMapSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 6.5), "${ChatColor.GOLD}▲△▲")
    private val leaderboardMapCurrent = ArmorStandUtil.createHologram(instance.getMapSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 5.5), "") // GOLD + BOLD + map name
    val leaderboardMapDownArrow = ArmorStandUtil.createHologram(instance.getMapSelectorLocation().or(LOCATION_ZERO).addY(ArmorStandUtil.offset * 4.5), "${ChatColor.GOLD}▼▽▼")
